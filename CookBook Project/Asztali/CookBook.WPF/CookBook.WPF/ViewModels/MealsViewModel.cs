...
namespace CookBook.WPF.ViewModels
{
    public class MealsViewModel : PagerViewModel, IEditCommand<Meal>
    {
        ...
        private async Task Save(Meal meal)
        {
            ...
            // Ha létező elem, akkor módosítás (UPDATE)
            if (exist)
            {
                await _mealRepo.UpdateAsync(meal.Id, meal);
            }
            // Ha nem létezik, akkor beillesztés (INSERT)
            else
            {
                // TODO: mentés után lekérdezni az új ID-t
                // REPO művelet, hozzáadja az adatbázishoz
                await _mealRepo.InsertAsync(meal);
                // GUI művelet, hozzáadja a megjelenített listához
                Meals.Insert(0, meal);
            }
        }
